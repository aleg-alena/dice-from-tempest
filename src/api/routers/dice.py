...
async def dice(message: Message, uow: UnitOfWork) -> None:
    ...
    if Lang =='en':
        await message.reply(f"Successfully created a command with the id {new_room_id}")
    else:
...
async def join(message: Message, uow: UnitOfWork) -> None:
    ...
    if owner_data > user_data:
        ...
        await uow.users.remove_gain(message.from_user.id, gain)
        ...
        await uow.users.remove_gain(room.owner_id, gain)
        if Lang =='en':
